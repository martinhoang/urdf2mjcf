...
try:
    from urdf2mjcf.tools.extract_dae_meshes import extract_meshes_from_dae
...
def zero_inertial_orientation(link_node):
    ...
    try:
        ...
        print_debug(
            f"   -> Zeroed inertial orientation for link '{link_name}' (was: {roll:.4f} {pitch:.4f} {yaw:.4f})"
        )
...
def resolve_path(path):
    ...
    def _resolve_find_syntax(path_str, context=""):
        ...
        if find_match:
            ...
            print_info(
                f"Resolving for package '{package_name}' with relative path '{relative_path}'"
            )
            try:
                ...
                print_warning(
                    f"Could not resolve package '{package_name}'{context_msg}."
                )
                return None
        return None
    ...
    def _resolve_env_syntax(path_str, context=""):
        ...
        def replace_env_var(match):
            ...
            if env_value is not None:
                ...
                context_msg = f" in {context}" if context else ""
                print_warning(
                    f"Environment variable '{var_name}' not found{context_msg}."
                )
    ...
    for node in nodes[1:]:
        for child in node:
            ...
            for existing_child in merged:
                ...
                if (
                    existing_child.tag == child.tag
                    and existing_child.attrib == child.attrib
                    and (existing_child.text or "").strip()
                    == (child.text or "").strip()
                ):
...
def preprocess_urdf(
    ...
    ###########################
    absolute_mesh_paths = {}
    link_properties = {}  # Store mass and mesh scales for each link
    ...
    if zero_inertial_rpy:
        ...
        if transformed_count > 0:
            print_confirm(
                f"-> Transformed {transformed_count} inertial orientation(s) to zero RPY"
            )
    ...
    for link_node in link_nodes:
        ...
        if inertial_elem is not None:
            ...
            if mass_elem is not None:
                ...
                if mass_value:
                    try:
                        link_properties[link_name]["mass"] = float(mass_value)
                        print_debug(
                            f"   -> Found mass for link '{link_name}': {mass_value} kg"
                        )
                    except ValueError:
                        print_warning(
                            f"Invalid mass value '{mass_value}' for link '{link_name}'"
                        )
        ...
        for visual_idx, visual_elem in enumerate(visual_elements):
            ...
            if not src_path or not os.path.isfile(src_path):
                raise FileNotFoundError(
                    f"Mesh file at {file_path} not found or could not be resolved."
                )
            ...
            if file_ext == ".dae" and EXTRACT_DAE_AVAILABLE:
                print_info(
                    f"Expanding DAE file '{base_name}' for link '{link_name}' into multiple visual elements..."
                )
                ...
                try:
                    ...
                    if success_count > 0 and dae_mesh_info:
                        ...
                        for mesh_idx, (mesh_name, mesh_data) in enumerate(
                            dae_mesh_info.items()
                        ):
                            ...
                            if rgba:
                                ...
                                print_debug(
                                    f"   -> Added visual '{mesh_name}' with material (RGBA: {rgba_str})"
                                )
                            else:
                                print_debug(
                                    f"   -> Added visual '{mesh_name}' (no material info)"
                                )
                            ...
                            temp_mesh_path = os.path.join(
                                temp_dae_extract_dir, extracted_file
                            )
        ...
        for mesh_elem in collision_mesh_nodes:
            ...
            if not src_path or not os.path.isfile(src_path):
                raise FileNotFoundError(
                    f"Collision mesh file at {file_path} not found or could not be resolved."
                )
            ...
            link_properties[link_name]["mesh_scales"]["collision"].append(mesh_scale)
        ...
        if "visual" in link_dict and "collision" in link_dict:
            ...
            for i in range(len(vis_dest_paths)):
                ...
                collided_indices = [
                    j
                    for j, dest_collision in enumerate(col_dest_paths)
                    if dest_collision == dest_visual and col_src_paths[j] != src_visual
                ]

                if len(collided_indices) > 0:
                    # Rename visual mesh
                    vis_filename, visual_ext = os.path.splitext(
                        os.path.basename(dest_visual)
                    )
                    ...
                    for idx in collided_indices:
                        ...
                        col_filename, collision_ext = os.path.splitext(
                            os.path.basename(col_dest_path)
                        )
                        new_col_dest_path = f"{col_filename}_collision{collision_ext}"
                        col_dest_paths[idx] = new_col_dest_path

                        collision_mesh_elements = link_node.findall(
                            ".//collision/geometry/mesh"
                        )
                        if idx < len(collision_mesh_elements):
                            collision_mesh_elements[idx].set(
                                "filename", new_col_dest_path
                            )
